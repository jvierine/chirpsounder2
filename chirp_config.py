...
''',
        ...
        cf["lfm"] = {
            ...
            "serendipitous_publish_range_starts_km": "[]",
...
#        self.snr_threshold = json.loads(cf["config"]["snr_threshold"])
        ...
        self.serendipitous_publish_range_starts_km = [
            float(x) for x in json.loads(cf["lfm"]["serendipitous_publish_range_starts_km"])
        ]
...
#        print(cf["config"]["channel"])
    ...
    def serendipitous_range_start_publishable(self, start_km):
        if not self.serendipitous_publish_range_starts_km:
            return self.serendipitous_range_start_allowed(start_km)
        return any(abs(float(start_km) - publish_km) < 0.5
                   for publish_km in self.serendipitous_publish_range_starts_km)
